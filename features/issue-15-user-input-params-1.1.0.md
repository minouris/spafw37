...
**Implementation for Step 5.1:**

**Implementation order:**

1. Create module-level variable for global handler storage
2. Implement `_get_prompt_handler()` function with three-tier resolution
3. Add tests for param-level handler precedence
4. Add tests for global handler precedence
5. Add tests for default handler fallback

**Code 5.1.1: Module-level global handler variable**

```python
# Block 5.1.1: Add to src/spafw37/param.py at module level
# Module-level variable for global prompt handler
_global_prompt_handler = None
```

**Code 5.1.2: _get_prompt_handler function**

```python
# Block 5.1.2: Add to src/spafw37/param.py
def _get_prompt_handler(param_def):
    """Resolve prompt handler using three-tier precedence system.
    
    Checks param-specific handler first, then global handler, then default.
    This enables fine-grained customisation whilst maintaining sensible defaults.
    ...
    """
    # Block 5.1.2.1: Check param-level handler (highest precedence)
    if PARAM_PROMPT_HANDLER in param_def:
        param_handler = param_def[PARAM_PROMPT_HANDLER]
        if param_handler is not None:
            return param_handler
    
    # Block 5.1.2.2: Check global handler (middle precedence)
    if _global_prompt_handler is not None:
        return _global_prompt_handler
    
    # Block 5.1.2.3: Return default handler (lowest precedence)
    return _get_default_handler()
```

**Test 5.1.3: Tests for _get_prompt_handler with param-level handler**

```gherkin
Scenario: Param-level handler takes precedence
  Given a parameter with PARAM_PROMPT_HANDLER set
  And a global handler is also set
  When _get_prompt_handler is called
  Then it returns the param-level handler
  
  # Tests: Handler resolution precedence
  # Validates: Param-level handlers override global handlers
```

```python
# Test 5.1.3: Add to tests/test_param.py
def test_get_prompt_handler_param_level_precedence():
    """Test param-level handler takes precedence over global handler.
    
    This test verifies that when both param-level and global handlers are set,
    the param-level handler is returned.
    This behaviour is expected because param-level customisation should override
    application-wide settings for specific use cases.
    """
    setup_function()
    
    # Block 5.1.3.1: Define test handlers
    def param_specific_handler(param_def):
        return "param_handler_called"
    
    def global_handler(param_def):
        return "global_handler_called"
    
    # Block 5.1.3.2: Set global handler and create param with specific handler
    param._global_prompt_handler = global_handler
    test_param = {
        PARAM_NAME: 'test_param',
        PARAM_PROMPT_HANDLER: param_specific_handler
    }
    
    # Block 5.1.3.3: Verify param handler returned
    handler = param._get_prompt_handler(test_param)
    assert handler is param_specific_handler
```

**Test 5.1.4: Tests for _get_prompt_handler with global handler**

```gherkin
Scenario: Global handler used when no param-level handler
  Given a parameter without PARAM_PROMPT_HANDLER
  And a global handler is set
  When _get_prompt_handler is called
  Then it returns the global handler
  
  # Tests: Global handler fallback
  # Validates: Global handler used when param has no specific handler
```

```python
# Test 5.1.4: Add to tests/test_param.py
def test_get_prompt_handler_global_fallback():
    """Test global handler used when param has no specific handler.
    
    This test verifies that the global handler is returned when
    a parameter definition does not specify its own handler.
    This behaviour is expected because the global handler provides
    application-wide customisation for all params without specific handlers.
    """
    setup_function()
    
    # Block 5.1.4.1: Define and set global handler
    def global_handler(param_def):
        return "global_handler_called"
    
    param._global_prompt_handler = global_handler
    
    # Block 5.1.4.2: Create param without specific handler
    test_param = {PARAM_NAME: 'test_param'}
    
    # Block 5.1.4.3: Verify global handler returned
    handler = param._get_prompt_handler(test_param)
    assert handler is global_handler
```

**Test 5.1.5: Tests for _get_prompt_handler with default handler**

```gherkin
Scenario: Default handler used when no handlers configured
  Given a parameter without PARAM_PROMPT_HANDLER
  And no global handler is set
  When _get_prompt_handler is called
  Then it returns the default handler
  
  # Tests: Default handler fallback
  # Validates: Built-in handler used when no custom handlers configured
```

```python
# Test 5.1.5: Add to tests/test_param.py
def test_get_prompt_handler_default_fallback():
    """Test default handler used when no custom handlers configured.
    
    This test verifies that the default handler is returned when
    neither param-level nor global handlers are set.
    This behaviour is expected because the framework should provide
    ...
    # Block 5.1.5.2: Create param without specific handler
    test_param = {PARAM_NAME: 'test_param'}
    
    # Block 5.1.5.3: Get handler
    handler = param._get_prompt_handler(test_param)
    
    # Block 5.1.5.4: Verify default handler returned
    assert handler is not None
    assert callable(handler)
```

**Code 5.1.6: _get_default_handler helper**

```python
# Block 5.1.6: Add to src/spafw37/param.py
def _get_default_handler():
    """Get default prompt handler from input_prompt module.
    
    Returns:
        Default prompt handler function
    """
    # Block 5.1.6.1: Import and return default handler
    from spafw37 import input_prompt
    return input_prompt.prompt_for_value
```

**Test 5.1.7: Tests for _get_default_handler**

```gherkin
Scenario: Default handler imports input_prompt module
  When _get_default_handler is called
  Then it returns prompt_for_value from input_prompt module
  
  # Tests: Default handler resolution
  # Validates: Correct default handler is imported and returned
```

```python
# Test 5.1.7: Add to tests/test_param.py
def test_get_default_handler_returns_input_prompt():
    """Test default handler returns input_prompt.prompt_for_value.
    
    This test verifies that _get_default_handler returns the correct
    default handler function from the input_prompt module.
    This behaviour is expected because input_prompt provides the
    built-in terminal prompting functionality.
    """
    setup_function()
    
    # Block 5.1.7.1: Get default handler
    handler = param._get_default_handler()
    
    # Block 5.1.7.2: Verify it's the input_prompt handler
    from spafw37 import input_prompt
    assert handler is input_prompt.prompt_for_value
```
